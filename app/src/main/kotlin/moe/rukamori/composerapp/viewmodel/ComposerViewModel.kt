...
sealed interface ProjectsScreenState {
    ...
    data class Success(val model: ProjectsUiModel) : ProjectsScreenState
    data object Empty : ProjectsScreenState
    data class Error(@StringRes val messageResId: Int) : ProjectsScreenState
}

sealed interface ImportScreenState {
    data object Loading : ImportScreenState
    data class Success(val model: ImportUiModel) : ImportScreenState
    data object Empty : ImportScreenState
    data class Error(@StringRes val messageResId: Int) : ImportScreenState
}

sealed interface EditorScreenState {
    data object Loading : EditorScreenState
    data class Success(val model: EditorUiModel) : EditorScreenState
    data object Empty : EditorScreenState
    data class Error(@StringRes val messageResId: Int) : EditorScreenState
}

sealed interface SyncScreenState {
    data object Loading : SyncScreenState
    data class Success(val model: SyncUiModel) : SyncScreenState
    data object Empty : SyncScreenState
    data class Error(@StringRes val messageResId: Int) : SyncScreenState
}

sealed interface TimelineScreenState {
    data object Loading : TimelineScreenState
    data class Success(val model: TimelineUiModel) : TimelineScreenState
    data object Empty : TimelineScreenState
    data class Error(@StringRes val messageResId: Int) : TimelineScreenState
}

sealed interface PreviewScreenState {
    data object Loading : PreviewScreenState
    data class Success(val model: PreviewUiModel) : PreviewScreenState
    data object Empty : PreviewScreenState
    data class Error(@StringRes val messageResId: Int) : PreviewScreenState
}

sealed interface ExportScreenState {
    data object Loading : ExportScreenState
    data class Success(val model: ExportUiModel) : ExportScreenState
    data object Empty : ExportScreenState
    data class Error(@StringRes val messageResId: Int) : ExportScreenState
}

sealed interface SettingsScreenState {
    data object Loading : SettingsScreenState
    data class Success(val model: SettingsUiModel) : SettingsScreenState
    data object Empty : SettingsScreenState
    data class Error(@StringRes val messageResId: Int) : SettingsScreenState
}
...
class ComposerViewModel @Inject constructor(
    ...
    fun updateLineText(
        ...
        val project = activeProject ?: return
        pushUndo(project)
        val updated = project.updateLine(lineId) { line ->
    ...
    fun nudgeSelectedWord(deltaMs: Long) {
        ...
        val lineId = selection.lineId ?: return
        val wordId = selection.wordId ?: return
        val word = project.lines.firstOrNull { it.id == lineId }?.words?.firstOrNull { it.id == wordId } ?: return
    ...
    private fun updateProjectsState() {
        ...
        _projectsState.value = if (summaries.isEmpty()) {
    ...
    private fun updateImportState() {
        _importState.value = ImportScreenState.Success(
            ImportUiModel(
    ...
    private fun updateEditorState() {
        ...
        _editorState.value = if (project == null) {
    ...
    private fun updateSyncState() {
        val project = activeProject
        _syncState.value = if (project == null) {
    ...
    private fun updateTimelineState() {
        val project = activeProject
        _timelineState.value = if (project == null) {
    ...
    private fun updatePreviewState() {
        val project = activeProject
        _previewState.value = if (project == null) {
    ...
    private fun updateExportState() {
        val project = activeProject
        _exportState.value = if (project == null) {
    ...
    private fun ComposerProject.toLineUiCollection(): LineUiCollection =
        LineUiCollection.from(lines.map { it.toLineUiModel(agents) })
