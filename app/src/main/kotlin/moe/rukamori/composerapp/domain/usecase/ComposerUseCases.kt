...
class CreateProjectUseCase @Inject constructor(
    ...
    suspend operator fun invoke(title: String): ComposerProject =
        ComposerProject.empty(title).also { repository.saveProject(it) }
}
...
class ImportLyricsUseCase @Inject constructor(
    ...
    suspend operator fun invoke(
        ...
        parser.parse(text, format, title).also { repository.saveProject(it) }
}

class ImportAudioUseCase @Inject constructor(
    private val audioRepository: AudioRepository,
) {
    suspend operator fun invoke(uri: Uri) = audioRepository.inspectAudio(uri)
}

class ExportTtmlUseCase @Inject constructor(
...
class UpdateTimelineWordUseCase @Inject constructor() {
    operator fun invoke(
        ...
        return project.updateWord(lineId, wordId) { word ->
...
fun ComposerProject.touch(): ComposerProject =
    copy(metadata = metadata.copy(updatedAtMs = System.currentTimeMillis()))
...
fun ComposerProject.replaceLines(lines: List<ComposerLine>): ComposerProject =
    copy(lines = lines).touch()

fun ComposerProject.updateLine(
    lineId: String,
    transform: (ComposerLine) -> ComposerLine,
): ComposerProject =
    copy(lines = lines.map { if (it.id == lineId) transform(it) else it }).touch()
...
fun ComposerProject.mergeWordWithNext(
    ...
    updateLine(lineId) { line ->
        ...
        line.copy(
            text = line.words.mapIndexedNotNull { wordIndex, word ->
                when (wordIndex) {
                    index -> merged.text
                    index + 1 -> null
                    else -> word.text
            ...
            words = line.words.toMutableList().apply {
                set(index, merged)
                removeAt(index + 1)
            },
        )
